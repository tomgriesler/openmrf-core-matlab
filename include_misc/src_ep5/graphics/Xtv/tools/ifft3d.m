function dat = ifft3d( dat )
%3D inverse Fast Fourier Transform with MRI convention (k-space centre centred)
%
% image = ifft3d( data );
%
% input-size:      [X1  X2  X3]
...
% Michael.Voelker@mr-bavaria.de, 2012
    ...
    if havePlentyRAM
        ...
        dat = cmshiftnd(  ifft(ifft(ifft(  cmshiftnd( dat, shift ),  [],Ndim-2),[],Ndim-1),[],Ndim),  shift );
    ...
    switch Ndim
        ...
        case 4
            % fine
        otherwise
            error( 'ifft3d:WrongNumDim', 'Input with 3 or 4 dimensions, please, with ifft-dimensions at the end.' )
    end
