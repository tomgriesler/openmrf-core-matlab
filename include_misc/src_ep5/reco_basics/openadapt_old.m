function [ im, cmap, wfull] = openadapt_old( im, doNorm, Rn, wfull, zf, messageString )
...
% ----- basic input handling -------------------------------------------
    if ndims(im) > 4 || ismatrix(im) || ~isnumeric(im) || any( ~isfinite(im(:)) )
...
% ----- noise handling -------------------------------------------------
    if bNoise
        ...
        clear  eieiei  L  n  maxN  normR

    end % of noise handling
% ----------------------------------------------------------------------

precis = class(im);                     % single or double precision?
wantCmaps = nargout > 1 || (doNorm && ~weights_present);    % are we interested in coil maps?
...
if wantCmaps && bNoise      % otherwise we just don't need it
    % preallocation
    cmapsmall = complex(zeros( nc, NstepY, NstepX, NstepZ, precis ));
end
...
if ~weights_present || wantCmaps
    ...
    % preallocation:
    wsmall = complex(zeros( nc, NstepY, NstepX, NstepZ, precis ));   % low resolution weights

    parfor x = 1:NstepX      % different x-coordinates are treated simultaneously on several workers

      % helper variables to make parfor-loop work (in Matlab-Babbel, we need "sliced" variables)
      WsmTmp = complex(zeros( nc, NstepY, NstepZ, precis ));    % temporary variable, holding a row of wsmall for one specific y-index
      ClmTmp = WsmTmp;                                          % temporary variable, holding a row of cmapsmall for one specific y-index

      % Work with a reduced image stripe with side length bsX in x-direction
      % Due to this we can work with sequential pieces of memory in the inner loops
      imBlockX = im(:,:,xmin(x):xmax(x),:);                             %#ok <-- suppress a warning in matlab editor


      for y = 1:NstepY

          imBlockY = imBlockX(:,ymin(y):ymax(y),:,:);                       %#ok <-- suppress a warning in matlab editor

          for z = 1:NstepZ

            m = reshape( imBlockY(:,:,:,zmin(z):zmax(z)), nc, [] );         %#ok <-- suppress a warning in matlab editor
            ...
            if bNoise
                WsmTmp(:,y,z) = normmf / (normmf' * (Rn \ normmf));   % MiVö: Should the denomninator not be sqrt(normmf' * (Rn \ normmf)) ? --> Eq. (20) in the Walsh-Paper
            else
                WsmTmp(:,y,z) = normmf;
            end
    ...
    wsmall = reshape( wsmall(:,:).', NstepY, NstepX, NstepZ, nc );      % NY x NX x NZ x NC
    wsmall = bsxfun( @times, wsmall, exp( -1i .* angle(wsmall(:,:,:,maxcoil)) ) );

    if wantCmaps && bNoise
        %cmapsmall = permute( cmapsmall, [2 3 4 1] );  % NY x NX x NZ x NC
        cmapsmall = reshape( cmapsmall(:,:).', NstepY, NstepX, NstepZ, nc );      % NY x NX x NZ x NC
    end
