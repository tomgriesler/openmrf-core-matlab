...
function [k,g,s,m1,m2,t,v]=vds_plotgradinfo(g,T,k0)
...
[k,g,s,m1,m2,t,v]=vds_calcgradinfo(g,T,k0);
...
subplot(M,N,2);
vds_doplot(t,k,'k',pls,acq);
xlabel('time(ms)');
ylabel('K-space (cm^{-1})');
title('K-space vs time');

subplot(M,N,3);
vds_doplot(t,g,'g',pls,acq);
xlabel('time(ms)');
ylabel('Gradient (G/cm)');
title('Gradient vs time');

subplot(M,N,4);
vds_doplot(t,s,'slew',pls,acq);
xlabel('time(ms)');
...
if (0==0)	% Plot second moment
  ...
  if (M*N>=6)
    subplot(M,N,6);
    vds_doplot(t,v,'voltage',pls,acq);
    xlabel('time(ms)');
    ylabel('Voltage (V)');
