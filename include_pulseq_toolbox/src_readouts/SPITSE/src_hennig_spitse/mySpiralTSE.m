...
%    try seq=mr.Sequence(system); end
    ...
    if(setpar==1)
        ...
        acqP.MTfac=1;
        SLfac=1;
        acqP.sliceThickness=4e-3*SLfac; acqP.sliceGAP=1.25/SLfac; acqP.nex=1;
        seg.n_acc=1.4;
        seg.n_full=1;
        ...
        if(strcmp(seqvar_mod,'T1var'))
            ...
            acqP.nrep=1;
            %         if(strcmp(T2prep,'on'))
            %             if(acqP.TEeff<acqP.TEprep)
            %                 acqP.nTE=ceil(acqP.TEprep/acqP.TE);
            %                 acqP.TEeff=acqP.nTE*acqP.TE;
            %                 disp(strcat('TEeff too short, corrected to :',num2str(acqP.TEeff)))
            %             else
            %                 acqP.nTE=acqP.nTE-floor(acqP.TEprep/acqP.TE)+1;
            %                 acqP.TEeff=acqP.TEprep+(acqP.nTE-1)*acqP.TE;
            %             end
            %         end
            %%
            acqP.necho=10;
        ...
        %         switch seqvar_mod
        %             case 'TEvar'
        %                 seqname=strcat(seqtype,num2str(round(1000*acqP.TE)),'_',num2str(floor(1000*acqP.TEeff)),'_',num2str(acqP.flipref),'_',num2str(acqP.NSlices));
        %             case 'accvar'
        ...
        %             case 'fsvar'
        %                 seqname=strcat(seqtype,num2str(round(1000*acqP.TE)),'_',num2str(floor(1000*acqP.TEeff)),'_',num2str(acqP.flipref),'_',fatsat,'_',num2str(acqP.NSlices));
        %
        %         end
        ...
        [rfex, gz] = mr.makeSincPulse(acqP.flipex,system,'Duration',segP.tEx,...
            'sliceThickness',acqP.sliceThickness,'apodization',0.5,'timeBwProduct',4,'use','excitation','maxSlew',system.maxSlew*4);
       ...
       [ktraj_adc, t_adc, ktraj, t_ktraj, t_excitation, t_refocusing] = seq.calculateKspacePP();
        ...
        seqname=strcat('TSEon_',num2str(seqno));
        %seqname=allname;
        ...
        %s=fileread(strcat(seqname,'TSE_T2var.seq'));
