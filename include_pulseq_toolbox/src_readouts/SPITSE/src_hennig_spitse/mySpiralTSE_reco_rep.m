...
%     seqname=strcat('p',seqname(1:end-4),'_1')
  % seqname='pTSE_off_10_80_150_11_6_2';
    ...
    ktraj_adc=reshape(ktraj_adc,[3,acq.nadc,necho,nslices*acqP.nrep]);
    close all
    %for kex=1:6,
...
%     end
    ...
    %for kex=1:1,
        ...
        %for nsl=1:1,
            ...
            if(strcmp(recnorm,'y'))
                ...
                [mph,amp]=pg_cpf([1 0 0 0],1,acqP.rflip,0,acqP.TE*1000,r1,r2,0);
            ...
            %        axis square
            imsos(:,:,nsl,kex)=abs(sos(im));
         %   imcoils(:,:,:,nsl)=im;
            %raw_sp_all(:,:,nsl,mp)=raw_sp;
        end
   %%  
   sim=size(imsos);
   temp=reshape(imsos,[sim(1) sim(2) sim(4) sim(3)]);
   imsos1=0*temp; 
        for k=1:nslices
            imsos1(:,:,:,slOrder(k))=temp(:,:,:,k);
  ...
  %          raw_sp_all(:,:,slOrder(k),mp)=raw_sp_all(:,:,k,mp);
      ...
      %  imcoila(:,:,:,:,kim,kex)=imcoils1;
       ...
       %bigx=im_mosaic(imsos1(:,:,2:end),2,5,150);
