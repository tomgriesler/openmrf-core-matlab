...
%% ---------- design numerical phantom, simulate Mxy(TR), simulate MRI signals ----------
...
phantom     = mg_numerical_phantom(Nxy, geo);
clear geo;

% add parameter noise
phantom.noise_level = 0.01; % [%]

% choose physical properties
[T1, T2]  = NIST_references('3.0T_MnCl2');
temp_ind  = [11; 10; 1; 7; 14; 3; 2; 12; 4; 9; 8; 5; 6; 13];
PD        = ones(1, phantom.n_phant + 1)';
T1        = [3.0; T1(temp_ind)];
T2        = [1.0; T2(temp_ind)];
phantom   = mg_phantom_properties(phantom, PD, T1, T2);
clear PD temp_ind;

% ---------- step 2: simulate Mxy for all voxels at beginning of all ADCs ----------
params_dict.P = phantom.p1D;
switch sim_mode
    case 'EPG'
...
DATA = mg_phantom_sim(Mxy .* repmat(phantom.p1D.PD,[1,NR]).', phantom.p2D.mask, phantom.p2D.cmaps, ktraj, proj_id, phantom.p2D.T2s, phantom.p2D.dw0, PULSEQ.SPI.adc.dwell, PULSEQ.FOV.fov_xy);
DATA = DATA / max(abs(DATA(:))) * max(abs(Mxy(:)));
clear Mxy;

% add noise to simulated MRI signal
rng("default")
noise_level_signal = 0.001; % [%]
DATA = DATA + (randn(size(DATA)) + 1i * randn(size(DATA))) * noise_level_signal / 100;
...
mask_inserts = phantom.p2D.ind>1;
mask_inserts = imerode(mask_inserts, strel('diamond', 4));

% T1
temp_dev_t1_direct = (match.direct.T1 - phantom.p2D.T1) ./ phantom.p2D.T1 * 100;
temp_dev_t1_direct = temp_dev_t1_direct(mask_inserts);
temp_dev_t1_SVD    = (match.SVD.T1 - phantom.p2D.T1) ./ phantom.p2D.T1 * 100;
temp_dev_t1_SVD    = temp_dev_t1_SVD(mask_inserts);
temp_dev_t1_LR     = (match.LR.T1 - phantom.p2D.T1) ./ phantom.p2D.T1 * 100;
...
% T2
temp_dev_t2_direct = (match.direct.T2 - phantom.p2D.T2) ./ phantom.p2D.T2 * 100;
temp_dev_t2_direct = temp_dev_t2_direct(mask_inserts);
temp_dev_t2_SVD    = (match.SVD.T2 - phantom.p2D.T2) ./ phantom.p2D.T2 * 100;
temp_dev_t2_SVD    = temp_dev_t2_SVD(mask_inserts);
temp_dev_t2_LR     = (match.LR.T2 - phantom.p2D.T2) ./ phantom.p2D.T2 * 100;
...
ax1 = subplot(3,3,1);
    imagesc(abs(match.direct.M0) .* phantom.p2D.mask); axis image; axis off; colormap(gca, gray); colorbar;
    title('M0 direct');
ax2 = subplot(3,3,2);
    imagesc(abs(match.SVD.M0) .* phantom.p2D.mask); axis image; axis off; colormap(gca, gray); colorbar;
    title('M0 SVD');
ax3 = subplot(3,3,3);
    imagesc(abs(match.LR.M0) .* phantom.p2D.mask); axis image; axis off; colormap(gca, gray); colorbar;
    title('M0 LR');

ax4 = subplot(3,3,4);
    imagesc(match.direct.T1 .* phantom.p2D.mask, t1lims); axis image; axis off; colormap(gca, t1cmp); colorbar;
    title('T1 direct');
ax5 = subplot(3,3,5);
    imagesc(match.SVD.T1 .* phantom.p2D.mask, t1lims); axis image; axis off; colormap(gca, t1cmp); colorbar;
    title('T1 SVD');
ax6 = subplot(3,3,6);
    imagesc(match.LR.T1 .* phantom.p2D.mask, t1lims); axis image; axis off; colormap(gca, t1cmp); colorbar;
    title('T1 LR');

ax7 = subplot(3,3,7);
    imagesc(match.direct.T2 .* phantom.p2D.mask, t2lims); axis image; axis off; colormap(gca, t2cmp); colorbar;
    title('T2 direct');
ax8 = subplot(3,3,8);
    imagesc(match.SVD.T2 .* phantom.p2D.mask, t2lims); axis image; axis off; colormap(gca, t2cmp); colorbar;
    title('T2 SVD');
ax9 = subplot(3,3,9);
    imagesc(match.LR.T2 .* phantom.p2D.mask, t2lims); axis image; axis off; colormap(gca, t2cmp); colorbar;
    title('T2 LR');
...
dT1_direct = (match.direct.T1 - phantom.p2D.T1) ./ phantom.p2D.T1 *100;
dT1_SVD    = (match.SVD.T1    - phantom.p2D.T1) ./ phantom.p2D.T1 *100;
dT1_LR     = (match.LR.T1     - phantom.p2D.T1) ./ phantom.p2D.T1 *100;
dT2_direct = (match.direct.T2 - phantom.p2D.T2) ./ phantom.p2D.T2 *100;
dT2_SVD    = (match.SVD.T2    - phantom.p2D.T2) ./ phantom.p2D.T2 *100;
dT2_LR     = (match.LR.T2     - phantom.p2D.T2) ./ phantom.p2D.T2 *100;

dT1_direct(phantom.p2D.mask==0) = 0;
dT1_SVD(phantom.p2D.mask==0)    = 0;
dT1_LR(phantom.p2D.mask==0)     = 0;
dT2_direct(phantom.p2D.mask==0) = 0;
dT2_SVD(phantom.p2D.mask==0)    = 0;
dT2_LR(phantom.p2D.mask==0)     = 0;
...
for j=1:phantom.n_phant
    temp = match.SVD.T1((phantom.p2D.ind==j+1).*mask_inserts==1);
    t1_vals_SVD(j,1) = mean(temp);
    t1_vals_SVD(j,2) = std(temp);
    temp = match.SVD.T2((phantom.p2D.ind==j+1).*mask_inserts==1);
    t2_vals_SVD(j,1) = mean(temp);
    t2_vals_SVD(j,2) = std(temp);
end
for j=1:phantom.n_phant
    temp = match.LR.T1((phantom.p2D.ind==j+1).*mask_inserts==1);
    t1_vals_LR(j,1) = mean(temp);
    t1_vals_LR(j,2) = std(temp);
    temp = match.LR.T2((phantom.p2D.ind==j+1).*mask_inserts==1);
...
plot(phantom.p.T1(2:phantom.n_phant+1), t1_vals_SVD(:,1), 'r.', 'MarkerSize', 25);
plot(phantom.p.T1(2:phantom.n_phant+1), t1_vals_LR(:,1),  'b.', 'MarkerSize', 25);
...
hold on;
plot([1e-4, 10], [1e-4, 10], 'k-', 'LineWidth', 2);
plot(phantom.p.T2(2:phantom.n_phant+1), t2_vals_SVD(:,1), 'r.', 'MarkerSize', 25);
plot(phantom.p.T2(2:phantom.n_phant+1), t2_vals_LR(:,1),  'b.', 'MarkerSize', 25);
plot([1e-4, 10], [1e-4, 10], 'k--', 'LineWidth', 2);
